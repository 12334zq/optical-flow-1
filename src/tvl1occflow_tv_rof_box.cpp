...
void
Scalar_ROF_BoxCellCentered(ofpix_t *u,
                           const ofpix_t *f,
                           ofpix_t *initialP1,
                           ofpix_t *initialP2,
                           const ofpix_t *g_function,
                           const double lambda,
                           ...
                           const int nIter)
{
    ...
    int jp1_v = 0;
    ofpix_t *stgGrid_F = 0;
    ofpix_t *stgGrid_P = 0; //where it will be placed p11 and p12
    ofpix_t *stgGrid_alfa = 0; //where it will be placed |grad(u1)|
    ofpix_t *ux = 0;
    ofpix_t *uy = 0;
    //Beta is from the Garamendi's paper, don't from coloma's paper
    ofpix_t *beta = 0; //Vector for storing the beta values of the 4x4 general matrix
    ...
    //if (stgGrid_F==NULL)
    {
        stgGrid_F = new ofpix_t[size_stg];
    }
    //if (stgGrid_P==NULL)
    {
        stgGrid_P = new ofpix_t[size_stg];
    }
    //if (stgGrid_alfa==NULL)
    {
        stgGrid_alfa = new ofpix_t[size_stg];
    }
