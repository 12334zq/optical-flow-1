...
static
double
cubic_interpolation_cell(double v[4],         //interpolation points
...
static
double
bicubic_interpolation_cell(double p[4][4],        //array containing the interpolation points
 ...
 **/
double
bicubic_interpolation_at(const ofpix_t *input,        //image to be interpolated
                                ...
                                )
{
    ...
    if (out && border_out) {
        ...
        //obtain the interpolation points of the image
        const ofpix_t p11 = input[(mx  + nx * my) * nz + k];
        const ofpix_t p12 = input[(x   + nx * my) * nz + k];
        const ofpix_t p13 = input[(dx  + nx * my) * nz + k];
        const ofpix_t p14 = input[(ddx + nx * my) * nz + k];
        const ofpix_t p21 = input[(mx  + nx * y) * nz + k];
        const ofpix_t p22 = input[(x   + nx * y) * nz + k];
        const ofpix_t p23 = input[(dx  + nx * y) * nz + k];
        const ofpix_t p24 = input[(ddx + nx * y) * nz + k];
        const ofpix_t p31 = input[(mx  + nx * dy) * nz + k];
        const ofpix_t p32 = input[(x   + nx * dy) * nz + k];
        const ofpix_t p33 = input[(dx  + nx * dy) * nz + k];
        const ofpix_t p34 = input[(ddx + nx * dy) * nz + k];
        const ofpix_t p41 = input[(mx  + nx * ddy) * nz + k];
        const ofpix_t p42 = input[(x   + nx * ddy) * nz + k];
        const ofpix_t p43 = input[(dx  + nx * ddy) * nz + k];
        const ofpix_t p44 = input[(ddx + nx * ddy) * nz + k];
...
} // bicubic_interpolation_at_color
 ...
 **/
void
bicubic_interpolation_warp(const ofpix_t *input,          // image to be warped
                           ...
                           )
{
    ...
    for (int i = 0; i < ny; i++) {
        for (int j = 0; j < nx; j++) {
            const int p = i * nx + j;
            const double uu = j + u[p];
            const double vv = i + v[p];
...
double
me_interpolate_bilinear(const ofpix_t *in,
                        int ncol,
                        double x,
                        double y)
{
...
double
me_interpolate_bicubic(const ofpix_t *in,
                       int nx,
                       int ny,
                       double x,
                       double y)
{
    ...
    return phi;
} // me_interpolate_bicubic

void
me_image_restriction(const ofpix_t *in,
                     ofpix_t *out,
                     int ncol,
                     int nrow,
                     int new_ncol,
