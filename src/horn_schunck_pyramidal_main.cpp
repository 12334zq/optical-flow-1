...
static
int
read_image(const char *fname, // file name of the image
     ...
     char *argv[])
{
    ...
    double alpha   = (argc >= 6) ?  atof(argv[i]) : PAR_DEFAULT_ALPHA;   i++;
    int nscales = (argc >= 7) ?  atoi(argv[i]) : PAR_DEFAULT_NSCALES; i++;
    double zfactor = (argc >= 8) ?  atof(argv[i]) : PAR_DEFAULT_ZFACTOR; i++;
    int warps   = (argc >= 9) ?  atoi(argv[i]) : PAR_DEFAULT_NWARPS;  i++;
    double TOL     = (argc >= 10) ? atof(argv[i]) : PAR_DEFAULT_TOL;     i++;
    ...
    if (zfactor >= 1) {
        zfactor = PAR_MAX_ZFACTOR;
    }
    if (warps   <= 0) {
        warps   = PAR_DEFAULT_NWARPS;
    }
    if (TOL     <= 0) {
        TOL     = PAR_DEFAULT_TOL;
    }
    ...
    if ( correct1 && correct2 && (nx == nx1) && (ny == ny1) ) {
        ...
        const double N = 1 + log(hypot(nx, ny) / 16) / log(1 / zfactor);
        if (N < nscales) {
            nscales = N;
        }
