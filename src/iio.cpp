...
#  define IIO_DEBUG(...) do { do_nop("", __VA_ARGS__); } \
    while (0)                                                    /* no-res */
...
normalize_type(int type_in)
{
    ...
    switch (type_in) {
    case IIO_TYPE_CHAR:
        type_out = IIO_TYPE_UINT8; break;
    case IIO_TYPE_SHORT:
        type_out = IIO_TYPE_INT16; break;
    case IIO_TYPE_INT:
        type_out = IIO_TYPE_UINT32; break;
    default:
        type_out = type_in; break;
    }
    if (type_out != type_in) {
...
static
int
iio_type_id(size_t sample_size,
            bool ieeefp_sample,
            bool signed_sample)
{
    if (ieeefp_sample) {
              ...
              int src_fmt)
{
    ...
    case CC(I8, U8):
        *(  int8_t*)dest = *( uint8_t*)src; break;
    case CC(I6, U6):
        *( int16_t*)dest = *(uint16_t*)src; break;
    case CC(I2, U2):
        *( int32_t*)dest = *(uint32_t*)src; break;
    case CC(U8, I8):
        *( uint8_t*)dest = *(  int8_t*)src; break;
    case CC(U6, I6):
        *(uint16_t*)dest = *( int16_t*)src; break;
    case CC(U2, I2):
        *(uint32_t*)dest = *( int32_t*)src; break;

    // different size signed integers (3 lossy, 3 lossless)
    case CC(I8, I6):
        *(  int8_t*)dest = *( int16_t*)src; break;            //iw810
    case CC(I8, I2):
        *(  int8_t*)dest = *( int32_t*)src; break;            //iw810
    case CC(I6, I2):
        *( int16_t*)dest = *( int32_t*)src; break;            //iw810
    case CC(I6, I8):
        *( int16_t*)dest = *(  int8_t*)src; break;
    case CC(I2, I8):
        *( int32_t*)dest = *(  int8_t*)src; break;
    case CC(I2, I6):
        *( int32_t*)dest = *( int16_t*)src; break;

    // different size unsigned integers (3 lossy, 3 lossless)
    case CC(U8, U6):
        *( uint8_t*)dest = *(uint16_t*)src; break;            //iw810
    case CC(U8, U2):
        *( uint8_t*)dest = *(uint32_t*)src; break;            //iw810
    case CC(U6, U2):
        *(uint16_t*)dest = *(uint32_t*)src; break;            //iw810
    case CC(U6, U8):
        *(uint16_t*)dest = *( uint8_t*)src; break;
    case CC(U2, U8):
        *(uint32_t*)dest = *( uint8_t*)src; break;
    case CC(U2, U6):
        *(uint32_t*)dest = *(uint16_t*)src; break;

    // diferent size mixed integers, make signed (3 lossy, 3 lossless)
    case CC(I8, U6):
        *(  int8_t*)dest = *(uint16_t*)src; break;            //iw810
    case CC(I8, U2):
        *(  int8_t*)dest = *(uint32_t*)src; break;            //iw810
    case CC(I6, U2):
        *( int16_t*)dest = *(uint32_t*)src; break;            //iw810
    case CC(I6, U8):
        *( int16_t*)dest = *( uint8_t*)src; break;
    case CC(I2, U8):
        *( int32_t*)dest = *( uint8_t*)src; break;
    case CC(I2, U6):
        *( int32_t*)dest = *(uint16_t*)src; break;

    // diferent size mixed integers, make unsigned (3 lossy, 3 lossless)
    case CC(U8, I6):
        *( uint8_t*)dest = *( int16_t*)src; break;            //iw810
    case CC(U8, I2):
        *( uint8_t*)dest = *( int32_t*)src; break;            //iw810
    case CC(U6, I2):
        *(uint16_t*)dest = *( int32_t*)src; break;            //iw810
    case CC(U6, I8):
        *(uint16_t*)dest = *(  int8_t*)src; break;
    case CC(U2, I8):
        *(uint32_t*)dest = *(  int8_t*)src; break;
    case CC(U2, I6):
        *(uint32_t*)dest = *( int16_t*)src; break;

    // from float (typically lossy, except for small integers)
    case CC(I8, F4):
        *(  int8_t*)dest = *( float*)src; break;            //iw810
    case CC(I6, F4):
        *( int16_t*)dest = *( float*)src; break;            //iw810
    case CC(I2, F4):
        *( int32_t*)dest = *( float*)src; break;
    case CC(I8, F8):
        *(  int8_t*)dest = *(double*)src; break;            //iw810
    case CC(I6, F8):
        *( int16_t*)dest = *(double*)src; break;            //iw810
    case CC(I2, F8):
        *( int32_t*)dest = *(double*)src; break;            //iw810
    case CC(U8, F4):
        *( uint8_t*)dest = T8(0.5 + *( float*)src); break;          //iw810
    case CC(U6, F4):
        *(uint16_t*)dest = T6(0.5 + *( float*)src); break;          //iw810
    case CC(U2, F4):
        *(uint32_t*)dest = *( float*)src; break;
    case CC(U8, F8):
        *( uint8_t*)dest = T8(0.5 + *(double*)src); break;          //iw810
    case CC(U6, F8):
        *(uint16_t*)dest = T6(0.5 + *(double*)src); break;          //iw810
    case CC(U2, F8):
        *(uint32_t*)dest = *(double*)src; break;            //iw810

    // to float (typically lossless, except for large integers)
    case CC(F4, I8):
        *( float*)dest = *(  int8_t*)src; break;
    case CC(F4, I6):
        *( float*)dest = *( int16_t*)src; break;
    case CC(F4, I2):
        *( float*)dest = *( int32_t*)src; break;            //iw810
    case CC(F8, I8):
        *(double*)dest = *(  int8_t*)src; break;
    case CC(F8, I6):
        *(double*)dest = *( int16_t*)src; break;
    case CC(F8, I2):
        *(double*)dest = *( int32_t*)src; break;
    case CC(F4, U8):
        *( float*)dest = *( uint8_t*)src; break;
    case CC(F4, U6):
        *( float*)dest = *(uint16_t*)src; break;
    case CC(F4, U2):
        *( float*)dest = *(uint32_t*)src; break;            //iw810
    case CC(F8, U8):
        *(double*)dest = *( uint8_t*)src; break;
    case CC(F8, U6):
        *(double*)dest = *(uint16_t*)src; break;
    case CC(F8, U2):
        *(double*)dest = *(uint32_t*)src; break;

#ifdef I_CAN_HAS_INT64
    // to int64_t and uint64_t
    case CC(I4, I8):
        *( int64_t*)dest = *(  int8_t*)src; break;
    case CC(I4, I6):
        *( int64_t*)dest = *( int16_t*)src; break;
    case CC(I4, I2):
        *( int64_t*)dest = *( int32_t*)src; break;
    case CC(I4, U8):
        *( int64_t*)dest = *( uint8_t*)src; break;
    case CC(I4, U6):
        *( int64_t*)dest = *(uint16_t*)src; break;
    case CC(I4, U2):
        *( int64_t*)dest = *(uint32_t*)src; break;
    case CC(I4, F4):
        *( int64_t*)dest = *(   float*)src; break;
    case CC(I4, F8):
        *( int64_t*)dest = *(  double*)src; break;
    case CC(U4, I8):
        *(uint64_t*)dest = *(  int8_t*)src; break;
    case CC(U4, I6):
        *(uint64_t*)dest = *( int16_t*)src; break;
    case CC(U4, I2):
        *(uint64_t*)dest = *( int32_t*)src; break;
    case CC(U4, U8):
        *(uint64_t*)dest = *( uint8_t*)src; break;
    case CC(U4, U6):
        *(uint64_t*)dest = *(uint16_t*)src; break;
    case CC(U4, U2):
        *(uint64_t*)dest = *(uint32_t*)src; break;
    case CC(U4, F4):
        *(uint64_t*)dest = *(   float*)src; break;
    case CC(U4, F8):
        *(uint64_t*)dest = *(  double*)src; break;

    // from int64_t and uint64_t
    case CC(I8, I4):
        *(  int8_t*)dest = *( int64_t*)src; break;
    case CC(I6, I4):
        *( int16_t*)dest = *( int64_t*)src; break;
    case CC(I2, I4):
        *( int32_t*)dest = *( int64_t*)src; break;
    case CC(U8, I4):
        *( uint8_t*)dest = *( int64_t*)src; break;
    case CC(U6, I4):
        *(uint16_t*)dest = *( int64_t*)src; break;
    case CC(U2, I4):
        *(uint32_t*)dest = *( int64_t*)src; break;
    case CC(F4, I4):
        *(   float*)dest = *( int64_t*)src; break;
    case CC(F8, I4):
        *(  double*)dest = *( int64_t*)src; break;
    case CC(I8, U4):
        *(  int8_t*)dest = *(uint64_t*)src; break;
    case CC(I6, U4):
        *( int16_t*)dest = *(uint64_t*)src; break;
    case CC(I2, U4):
        *( int32_t*)dest = *(uint64_t*)src; break;
    case CC(U8, U4):
        *( uint8_t*)dest = *(uint64_t*)src; break;
    case CC(U6, U4):
        *(uint16_t*)dest = *(uint64_t*)src; break;
    case CC(U2, U4):
        *(uint32_t*)dest = *(uint64_t*)src; break;
    case CC(F4, U4):
        *(   float*)dest = *(uint64_t*)src; break;
    case CC(F8, U4):
        *(  double*)dest = *(uint64_t*)src; break;
#endif//I_CAN_HAS_INT64

#ifdef I_CAN_HAS_LONGDOUBLE
    // to longdouble
    case CC(F6, I8):
        *(longdouble*)dest = *(  int8_t*)src; break;
    case CC(F6, I6):
        *(longdouble*)dest = *( int16_t*)src; break;
    case CC(F6, I2):
        *(longdouble*)dest = *( int32_t*)src; break;
    case CC(F6, U8):
        *(longdouble*)dest = *( uint8_t*)src; break;
    case CC(F6, U6):
        *(longdouble*)dest = *(uint16_t*)src; break;
    case CC(F6, U2):
        *(longdouble*)dest = *(uint32_t*)src; break;
    case CC(F6, F4):
        *(longdouble*)dest = *(   float*)src; break;
    case CC(F6, F8):
        *(longdouble*)dest = *(  double*)src; break;

    // from longdouble
    case CC(I8, F6):
        *(  int8_t*)dest = *(longdouble*)src; break;
    case CC(I6, F6):
        *( int16_t*)dest = *(longdouble*)src; break;
    case CC(I2, F6):
        *( int32_t*)dest = *(longdouble*)src; break;
    case CC(U8, F6):
        *( uint8_t*)dest = T8(0.5 + *(longdouble*)src); break;
    case CC(U6, F6):
        *(uint16_t*)dest = T6(0.5 + *(longdouble*)src); break;
    case CC(U2, F6):
        *(uint32_t*)dest = *(longdouble*)src; break;
    case CC(F4, F6):
        *(   float*)dest = *(longdouble*)src; break;
    case CC(F8, F6):
        *(  double*)dest = *(longdouble*)src; break;

#ifdef I_CAN_HAS_INT64 //(nested)
    case CC(F6, I4):
        *(longdouble*)dest = *( int64_t*)src; break;
    case CC(F6, U4):
        *(longdouble*)dest = *(uint64_t*)src; break;
    case CC(I4, F6):
        *( int64_t*)dest = *(longdouble*)src; break;
    case CC(U4, F6):
        *(uint64_t*)dest = *(longdouble*)src; break;
...
static void
unpack_nibbles_to_bytes(uint8_t out[2],
                        uint8_t in)
{
...
static void
unpack_couples_to_bytes(uint8_t out[4],
                        uint8_t in)
{
...
static void
unpack_bits_to_bytes(uint8_t out[8],
                     uint8_t in)
{
    out[7] = (in & 1) ? 1 : 0;
    out[6] = (in & 2) ? 1 : 0;
...
static void
unpack_to_bytes_here(uint8_t *dest,
                     uint8_t *src,
                     int n,
                     int bits)
{
    ...
    switch (bits) {
    case 1:
        FORI(n)    unpack_bits_to_bytes(dest + 8 * i, src[i]); break;
    case 2:
        FORI(n) unpack_couples_to_bytes(dest + 4 * i, src[i]); break;
    case 4:
        FORI(n) unpack_nibbles_to_bytes(dest + 2 * i, src[i]); break;
    default:
...
static void
iio_convert_samples(struct iio_image *x,
                    int desired_type)
{
...
static void
iio_hacky_colorize(struct iio_image *x,
                   int pd)
{
    assert(!x->contiguous_data);
    ...
    char *data_dest = (char*)rdata;
    char *data_src = (char*)x->data;
    FORI(n)
    FORJ(pd)
...
// Implementation: re-invent the wheel
static char *
put_data_into_temporary_file(void *filedata,
                             size_t filesize)
{
...
static void *
wrap_2dmatrix_around_data(void *x,
                          int w,
                          int h,
                          size_t s)
{
...
static void
break_pixels_float(float *broken,
                   float *clear,
                   int n,
                   int pd)
{
...
static void
recover_broken_pixels_float(float *clear,
                            float *broken,
                            int n,
                            int pd)
{
...
static int
read_beheaded_png(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
...
static int
read_whole_jpeg(struct iio_image *x,
                FILE *f)
{
...
static void
llegeix_floats_en_bytes(FILE *don,
                        float *on,
                        int quants)
{
...
static void
llegeix_floats_en_shorts(FILE *don,
                         float *on,
                         int quants)
{
    for (int i = 0; i < quants; i++) {
...
static void
llegeix_floats_en_ascii(FILE *don,
                        float *on,
                        int quants)
{
    for (int i = 0; i < quants; i++) {
...
// 19 Q9 (binary 3d nd           )
static int
read_beheaded_qnm(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
...
static int
read_beheaded_pcm(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
...
static int
read_beheaded_rim_cimage(struct iio_image *x,
                         FILE *f,
                         bool swp)
{
...
static int
read_beheaded_rim_fimage(struct iio_image *x,
                         FILE *f,
                         bool swp)
{
...
static int
read_beheaded_rim_ccimage(struct iio_image *x,
                          FILE *f,
                          bool swp)
{
    ...
    return 0;
} // read_beheaded_rim_ccimage

static int
read_beheaded_rim(struct iio_image *x,
                  FILE *f,
                  char *h,
                  int nh)
{
...
// PFM reader {{{2
static int
read_beheaded_pfm(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
...
// FLO reader {{{2
static int
read_beheaded_flo(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
...
// JUV reader {{{2
static int
read_beheaded_juv(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
    ...
    float *u = (float *)xmalloc(w * h * sf); r = fread(u, sf, w * h, f); if (r != w * h) {
        return -2;
    }
    float *v = (float *)xmalloc(w * h * sf); r = fread(v, sf, w * h, f); if (r != w * h) {
        return -2;
    }
...
static int
read_beheaded_lum(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
...
// BMP reader {{{2
static int
read_beheaded_bmp(struct iio_image *x,
                  FILE *f,
                  char *header,
                  int nheader)
{
    long len;
    char *bmp = (char *)load_rest_of_file(&len, f, header, nheader);
...
static int
read_whole_exr(struct iio_image *x,
               char *filename)
{
...
// by recovering the original filename through hacks, in case it was not a pipe.
static int
read_beheaded_exr(struct iio_image *x,
                  FILE *fin,
                  char *header,
                  int nheader)
{
...
static void
iio_save_image_as_png(const char *filename,
                      struct iio_image *x)
{
    ...
    png_bytepp row = (png_bytepp)xmalloc( height * sizeof(png_bytep) );
...
static void
iio_save_image_as_tiff(const char *filename,
                       struct iio_image *x)
{
    ...
    switch (x->type) {
    case IIO_TYPE_FLOAT:
        tsf = SAMPLEFORMAT_IEEEFP; break;
    case IIO_TYPE_INT8:
    case IIO_TYPE_INT16:
    case IIO_TYPE_INT32:
        tsf = SAMPLEFORMAT_INT; break;
    case IIO_TYPE_UINT8:
    case IIO_TYPE_UINT16:
    case IIO_TYPE_UINT32:
        tsf = SAMPLEFORMAT_UINT; break;
    default:
    ...
    TIFFClose(tif);
} // iio_save_image_as_tiff

static void
iio_save_image_as_tiff_smarter(const char *filename,
                               struct iio_image *x)
{
...
// JUV writer {{{2
static void
iio_save_image_as_juv(const char *filename,
                      struct iio_image *x)
{
    ...
    int w = x->sizes[0];
    int h = x->sizes[1];
    float *uv = (float *)x->data;
    float *u = (float *)xmalloc(w * h * sf); FORI(w * h) u[i] = uv[2 * i];
    float *v = (float *)xmalloc(w * h * sf); FORI(w * h) v[i] = uv[2 * i + 1];
    FILE *f = xfopen(filename, "w");
    fwrite(buf, 1, 255, f);
    fwrite(u, sf, w * h, f);
    fwrite(v, sf, w * h, f);
...
// FLO writer {{{2
static void
iio_save_image_as_flo(const char *filename,
                      struct iio_image *x)
{
    assert(x->type == IIO_TYPE_FLOAT);
    assert(x->dimension == 2);
...
static void
iio_save_image_as_rim_fimage(const char *fname,
                             struct iio_image *x)
{
...
static void
iio_save_image_as_rim_cimage(const char *fname,
                             struct iio_image *x)
{
...
static
int
read_beheaded_image(struct iio_image *x,
                    FILE *f,
                    char *h,
                    int hn,
                    int fmt)
...
//
static int
read_image_f(struct iio_image *x,
             FILE *f)
{
...
static
void *
iio_read_image(const char *fname,
               int *w,
               int *h,
               int desired_sample_type)
{
...
// API 2D
float *
iio_read_image_float_vec(const char *fname,
                         int *w,
                         int *h,
                         int *pd)
{
    ...
    if (r) {
        return (float *)rfail("could not read image");
    }
    if (x->dimension != 2) {
        x->dimension = 2;
        //error("non 2d image");
...
// API 2D
float *
iio_read_image_float_split(const char *fname,
                           int *w,
                           int *h,
                           int *pd)
{
    float *r = iio_read_image_float_vec(fname, w, h, pd);
    float *rbroken = (float *)xmalloc(*w * *h * *pd * sizeof *rbroken);
...
// API 2D
float *
iio_read_image_float_rgb(const char *fname,
                         int *w,
                         int *h)
{
    struct iio_image x[1];
    ...
    if (x->dimension != 2) {
        ...
        return (float *)rfail("non 2d image");
    }
    if (x->pixel_dimension != 3) {
...
// API 2D
double *
iio_read_image_double_vec(const char *fname,
                          int *w,
                          int *h,
                          int *pd)
{
    ...
    if (r) {
        return (double *)rfail("could not read image");
    }
    if (x->dimension != 2) {
        x->dimension = 2;
    }
...
// API 2D
uint8_t *
iio_read_image_uint8_vec(const char *fname,
                         int *w,
                         int *h,
                         int *pd)
{
    ...
    if (r) {
        return (uint8_t *)rfail("could not read image");
    }
    if (x->dimension != 2) {
        x->dimension = 2;
    }
...
// API 2D
uint16_t *
iio_read_image_uint16_vec(const char *fname,
                          int *w,
                          int *h,
                          int *pd)
{
    ...
    if (r) {
        return (uint16_t *)rfail("could not read image");
    }
    if (x->dimension != 2) {
        x->dimension = 2;
    }
...
uint8_t ( *iio_read_image_uint8_rgb(const char *fname, int *w, int *h) )[3]
{
    ...
    int r = read_image(x, fname);
    if (r) {
        return (uint8_t (*)[3])rfail("could not read image");
    }
    if (x->dimension != 2) {
        x->dimension = 2;
        fail("non 2d image");
...
uint8_t ( **iio_read_image_uint8_matrix_rgb(const char *fnam, int *w, int *h) )[3]
{
    ...
    if (x->dimension != 2) {
        ...
        return (uint8_t (**)[3])rfail("non 2d image");
    }
    if (x->pixel_dimension != 3) {
        iio_hacky_colorize(x, 3);
    }
    ...
    *h = x->sizes[1];
    ...
    return (uint8_t (**)[3])wrap_2dmatrix_around_data(x->data, *w, *h, 3);
}
...
float ( **iio_read_image_float_matrix_rgb(const char *fnam, int *w, int *h) )[3]
{
    ...
    int r = read_image(x, fnam);
    if (r) {
        return (float (**)[3])rfail("could not read image");
    }
    if (x->dimension != 2) {
        x->dimension = 2;

        return (float (**)[3])rfail("non 2d image");
    }
    if (x->pixel_dimension != 3) {
        iio_hacky_colorize(x, 3);
    }
    ...
    *h = x->sizes[1];
    ...
    return (float (**)[3])wrap_2dmatrix_around_data( x->data, *w, *h, 3 * sizeof(float) );
}

// API 2D
uint8_t ***
iio_read_image_uint8_matrix_vec(const char *fname,
                                int *w,
                                int *h,
                                int *pd)
{
    ...
    if (x->dimension != 2) {
        ...
        return (uint8_t ***)rfail("non 2d image");
    }
    *w = x->sizes[0];
    *h = x->sizes[1];
    *pd = x->pixel_dimension;
    ...
    return (uint8_t ***)wrap_2dmatrix_around_data(x->data, *w, *h, *pd);
}

// API 2D
void *
iio_read_image_float_matrix_vec(const char *fname,
                                int *w,
                                int *h,
                                int *pd)
{
...
// API 2D
uint8_t **
iio_read_image_uint8_matrix(const char *fname,
                            int *w,
                            int *h)
{
    struct iio_image x[1];
    ...
    *h = x->sizes[1];
    ...
    return (uint8_t **)wrap_2dmatrix_around_data(x->data, *w, *h, 1);
}

float **
iio_read_image_float_matrix(const char *fname,
                            int *w,
                            int *h)
{
    struct iio_image x[1];
    ...
    if (x->pixel_dimension != 1) {
        return (float **)rfail("non-scalar image");
    }
    *w = x->sizes[0];
    *h = x->sizes[1];
    iio_convert_samples(x, IIO_TYPE_FLOAT);

    return (float **)wrap_2dmatrix_around_data( x->data, *w, *h, sizeof(float) );
}
...
// API 2D
float *
iio_read_image_float(const char *fname,
                     int *w,
                     int *h)
{
    struct iio_image x[1];
    ...
    if (x->pixel_dimension != 1) {
        return (float *)rfail("non-scalarizable image");
    }
    *w = x->sizes[0];
    *h = x->sizes[1];
    iio_convert_samples(x, IIO_TYPE_FLOAT);
...
// API 2D
double *
iio_read_image_double(const char *fname,
                      int *w,
                      int *h)
{
    struct iio_image x[1];
    ...
    if (x->dimension != 2) {
        ...
        return (double *)rfail("non 2d image");
    }
    if (x->pixel_dimension == 3) {
        iio_hacky_uncolorize(x);
    }
    if (x->pixel_dimension != 1) {
        return (double *)rfail("non-scalar image");
    }
    *w = x->sizes[0];
    *h = x->sizes[1];
    iio_convert_samples(x, IIO_TYPE_DOUBLE);
...
// API 2D
uint8_t *
iio_read_image_uint8(const char *fname,
                     int *w,
                     int *h)
{
    return (uint8_t *)iio_read_image(fname, w, h, IIO_TYPE_UINT8);
}
...
static bool
these_floats_are_actually_bytes(float *t,
                                int n)
{
...
static void
iio_save_image_default(const char *filename,
                       struct iio_image *x)
{
    ...
    if ( (typ == IIO_TYPE_FLOAT) &&
         these_floats_are_actually_bytes( (float *)x->data, nsamp ) ) {
    ...
    if ( (x->pixel_dimension == 1) && (typ == IIO_TYPE_FLOAT) ) {
        int m = these_floats_are_actually_bytes( (float *)x->data, x->sizes[0] * x->sizes[1] ) ? 255 : 65535;
        fprintf(f, "P2\n%d %d\n%d\n", x->sizes[0], x->sizes[1], m);
        float *data = (float *)x->data;
        FORI(x->sizes[0] * x->sizes[1])
        fprintf(f, "%a\n", data[i]);
    } else if ( (x->pixel_dimension == 3) && (typ == IIO_TYPE_FLOAT) ) {
        int m = these_floats_are_actually_bytes( (float *)x->data, 3 * x->sizes[0] * x->sizes[1] ) ? 255 : 65535;
        float *data = (float*)x->data;
        fprintf(f, "P3\n%d %d\n%d\n", x->sizes[0], x->sizes[1], m);
    ...
    xfclose(f);
} // iio_save_image_default

void
iio_save_image_uint8_matrix_rgb(const char *filename,
                                uint8_t (**data)[3],
                                int w,
                                int h)
{
...
void
iio_save_image_uint8_matrix(const char *filename,
                            uint8_t **data,
                            int w,
                            int h)
{
...
void
iio_save_image_float_vec(const char *filename,
                         float *data,
                         int w,
                         int h,
                         int pd)
...
void
iio_save_image_float_split(const char *filename,
                           float *data,
                           int w,
                           int h,
                           int pd)
{
    float *rdata = (float *)xmalloc(w * h * pd * sizeof *rdata);
...
void
iio_save_image_double(const char *filename,
                      double *data,
                      int w,
                      int h)
{
...
void
iio_save_image_uint8_vec(const char *filename,
                         uint8_t *data,
                         int w,
                         int h,
                         int pd)
...
void
iio_save_image_uint16_vec(const char *filename,
                          uint16_t *data,
                          int w,
                          int h,
                          int pd)
